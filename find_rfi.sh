...
echo "Find and remove favicon_c45b08.ico"
find {SITE_PATH} -type f -name "favicon_*.ico" -exec rm -rfv {} \; && echo "OK" || exit 1

echo "Change permision"
find ${SITE_PATH} -type f -exec chmod 644 {} \; && echo "Change permision files OK" || exit 1
find ${SITE_PATH} -type d -exec chmod 755 {} \; && echo "Change permision dyrectory OK" || exit 1
